    ...
    knownHosts =
      ...
      in {
        hostNames =
          ...
          ++ (lib.optional (host ? ipv4) host.ipv4)
          ++ (lib.optionals (host ? et_ipv4) [host.et_ipv4 "${name}.et.${myvars.domain}"])
          ++ (lib.optional (host ? ipv6) host.ipv6)
          ++ (lib.optionals (host ? et_ipv6) [host.et_ipv6 "${name}.et.${myvars.domain}"]);
