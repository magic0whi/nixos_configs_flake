{pkgs, config, lib, mylib, ...}@args: let
  ...
  value_type = with lib.types; nullOr (oneOf [
    bool
    int
    float
    str
...
in {
  options.modules.gui.hyprland = {
    ...
    settings = lib.mkOption {
      type = value_type // {description = "Hyprland configuration value";};
      default = {};
    };
    nvidia = lib.mkEnableOption "Whether nvidia GPU is used exclusively";
    hypridle.settings = lib.mkOption {
      type = value_type // {description = "Hypridle configuration value";};
      default = {};
    };
  };
  config = lib.mkIf cfg.enable (lib.mkMerge ( # Merge multiple sets of option definitions together
    [
      ...
      {services.hypridle.settings = cfg.hypridle.settings;}
